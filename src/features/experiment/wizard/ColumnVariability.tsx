      ...
      <div>
        ...
        >
          <Box sx={style}>
            ...
            {initial_possible_params && params && params.filter(p => Array(initial_possible_params).indexOf(p.id)).map((f: any) => (
            <div>
                <Typography component="div">{f.description}:</Typography>
                <TextField id={f.id + ""} label={f.label} type={f.data_type} />
              </div>
            ))}
          </Box>
        </Modal>
